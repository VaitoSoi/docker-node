...
export class Secret {
    ...
    async list(filters) {
        try {
            const response = await this.api.get(`/secrets?` + objectToQuery({ filters }, {}, ['filters']));
    ...
    async create(option) {
        try {
            const response = await this.api.post(`/secrets`, option);
            return response.data;
        }
        catch (error) {
            if (axios.isAxiosError(error)) {
    ...
    async inspect(id) {
        try {
            const response = await this.api.get(`/secrets/${id}`);
            return response.data;
        }
        catch (error) {
            if (axios.isAxiosError(error)) {
