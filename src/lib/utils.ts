...
export function objectToQuery<Object extends Record<any, any> = Record<string, any>>(
    obj: Object, 
    replaceKey: Partial<Record<keyof Object, string>> = {},
    jsonFormatItems: (keyof Object)[] = [],
    ignoreItems: (keyof Object)[] = [],
) {
    const queryStrings: string[] = [];
    for (const [key, value] of Object.entries(obj)) 
        if (key in obj && obj[key] !== undefined && obj[key] !== null && obj[key] !== "" && !ignoreItems.includes(key))
            queryStrings.push(
                (key in replaceKey && !!replaceKey[key] ? replaceKey[key] : key) + 
                "=" + 
...
export function lt(v1: string, v2: string) {
    ...
    for (let index = 0; index < v1Split.length; index++) {
        ...
        if (!partV1 || !partV2) throw new InvalidInput();

        if (partV1 < partV2) return true;
        else if (partV1 > partV2) return false;
    }
    return false;
}
